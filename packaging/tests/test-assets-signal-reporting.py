...
def worker(driver, case):
    ...
    if case in ('expired-cleanup', 'partial-cleanup'):
        # Rescue opens BEFORE the fixtures exist. Round 5 of this arc found the
        # creation sitting above the try, so a fixture that started and then hit
        # an exception before the rescue scope was reached stayed running. That
        # window is not reachable at this revision -- nothing between creation
        # and the try can raise -- but the ordering is kept, because it was
        # reachable the moment anything was inserted there, which is exactly
        # what happened once.
        children = []
        try:
            children.extend(subprocess.Popen(['sleep', '100'])
                            for _ in range(2 if case == 'partial-cleanup' else 1))
